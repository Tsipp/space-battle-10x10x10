...
if (!state.gid) {
  document.body.innerHTML = `<main class="page"><div class="notice">Нужен параметр <code>?g=GAME_ID</code> в URL — запросите ссылку у ведущего (GM).</div></main>`;
  throw new Error("missing gid");
}
$("hdr-gid").textContent = state.gid;
...
function render(data) {
  ...
  $("you-label").textContent = data.you ? `${data.you.name} · ${roleLabel(data.you.role)}${data.you.team ? " · Team " + data.you.team : ""}` : "—";
...
function drawMap(data) {
  ...
  // Z selector in toolbar.
  const bar = $("map-toolbar");
  bar.innerHTML = "";
  for (let z = 0; z < 10; z++) {
    ...
    if (z === currentZ) btn.classList.add("primary");
    btn.addEventListener("click", () => { currentZ = z; render(data); });
    bar.appendChild(btn);
  }
}
